...
function squarify(items, rect) {
  ...
  let remaining = items.map((i) => ({
    ...i,
    area: (i.value / total) * rect.w * rect.h,
  }));
      ...
      <div className="heatmap-container" ref={containerRef}>
        <div className="heatmap-skeleton">
          {Array.from({ length: 11 }, (_, i) => (
            <div
              key={i}
              className={`skeleton-line heatmap-skeleton-cell heatmap-skeleton-cell--${i}`}
            />
          ))}
        </div>
      </div>
    ...
    >
      {dims.w > 0 &&
        sectorRects.map((sector) => {
          ...
          const industryItems = sector.industries
            .map((ind) => ({
              ...ind,
              value: ind.stocks.reduce(
          ...
          return (
            <div
              ...
              onMouseEnter={() =>
                !isMobileView && setHoveredSector(sector.name)
              }
            ...
            >
              ...
              {industryRects.map((ind) => {
                ...
                const visibleStocks = isMobileView
                  ? ind.stocks.slice(0, 6)
                  : ind.stocks;
                  ...
                  >
                    ...
                    {stockRects.map((st) => {
                      ...
                      const fontSize = Math.max(
                        6,
                        Math.min(14, Math.min(cellW / 4.5, cellH / 3.5)),
                      );
                      ...
                      return (
                        ...
                        >
                          {showTicker && (
                            <span
                              className="heatmap-cell-symbol"
                              style={{ fontSize: Math.max(6, fontSize + 1) }}
                            >
                              {st.symbol}
                            </span>
                          )}
                          ...
                          {showName && (
                            <span
                              className="heatmap-cell-name"
                              style={{ fontSize: Math.max(5, fontSize - 3) }}
                            >
                              {st.name}
                            </span>
                          )}
          ...
          {activeStock && (
            <div className="heatmap-tooltip-featured">
              <div className="heatmap-tooltip-featured-top">
                <span className="heatmap-tooltip-featured-symbol">
                  {activeStock.symbol}
                </span>
                <span className="heatmap-tooltip-featured-price">
                  {formatPrice(activeStock.price)}
                </span>
                <span
                  className={`heatmap-tooltip-featured-change ${activeStock.changePercent >= 0 ? "positive" : "negative"}`}
                >
                  {activeStock.changePercent >= 0 ? "+" : ""}
                  {activeStock.changePercent.toFixed(2)}%
                </span>
              </div>
              <div className="heatmap-tooltip-featured-name">
                {activeStock.name}
              </div>
            </div>
          )}
            ...
            <div key={ind.name}>
              ...
              {ind.stocks.map((st) => (
                ...
                >
                  ...
                  <span className="heatmap-tooltip-price">
                    {formatPrice(st.price)}
                  </span>
                  <span
                    className={`heatmap-tooltip-change ${st.changePercent >= 0 ? "positive" : "negative"}`}
                  >
                    {st.changePercent >= 0 ? "+" : ""}
                    {st.changePercent.toFixed(2)}%
                  </span>
