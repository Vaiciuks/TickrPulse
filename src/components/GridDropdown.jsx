  ...
  return (
    <div className="grid-dropdown" ref={ref}>
      <button className="grid-dropdown-btn" onClick={() => setOpen((o) => !o)}>
        ...
        <svg
          className={`grid-dropdown-arrow${open ? " grid-dropdown-arrow--open" : ""}`}
          width="10"
          height="10"
          viewBox="0 0 10 10"
          fill="none"
      ...
      {open && stocks.length > 0 && (
        <div className="grid-dropdown-panel">
          <div className="grid-dropdown-header">
            {label} <span>{stocks.length} stocks</span>
          </div>
            ...
            {stocks.slice(0, 30).map((stock) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="grid-dropdown-price">
                    {formatPrice(stock.price)}
                  </span>
                  <span
                    className={`grid-dropdown-change ${isPos ? "positive" : "negative"}`}
                  >
                    {isPos ? "+" : ""}
