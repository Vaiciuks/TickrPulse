...
export default function StockCard({ stock, chartData, onClick, isSelected, isFocused, hasNews, newsArticles, isFavorite, onToggleFavorite, hasNote, note, onSetNote }) {
  ...
  return (
    <article
      ref={cardRef}
      className={`stock-card ${flash || ''}${isSelected ? ' stock-card-selected' : ''}${isFocused ? ' stock-card-focused' : ''}`}
      onClick={onClick}
    >
      <div className="stock-card-header">
        <div className="stock-card-symbol-group">
          ...
          {hasNews && (
            <button className="stock-card-news-btn" onClick={handleNewsClick} aria-label={`News for ${stock.symbol}`} title="View news">
        ...
        <div className={`stock-card-change ${isPositive ? 'change-up-badge' : 'change-down-badge'}`}>
      ...
      <div className={`stock-card-price ${flash || ''}`}>{formatPrice(animatedPrice)}</div>
      {stock.extPrice != null && (
        <div className="stock-card-ext">
          <span className="stock-card-ext-label">{stock.extMarketState === 'pre' ? 'Pre' : 'AH'}</span>
          <span className="stock-card-ext-price">{formatPrice(stock.extPrice)}</span>
          <span className={`stock-card-ext-change ${stock.extChangePercent >= 0 ? 'change-up' : 'change-down'}`}>
      ...
      <div className="stock-card-meta">
        ...
        {stock.marketCap && <span>MCap {formatVolume(stock.marketCap)}</span>}
      </div>
      <div className="stock-card-chart">
        <MiniChart symbol={stock.symbol} isPositive={isPositive} data={chartData} />
      </div>
      {showNews && hasNews && (
        ...
        <div className="note-overlay" onClick={() => setShowNote(false)}>
          <div className="note-modal" onClick={e => e.stopPropagation()}>
            <div className="stock-note-popover-header">
              ...
              <button className="stock-note-popover-close" onClick={() => setShowNote(false)}>&times;</button>
            </div>
            <textarea
            ...
            <div className="stock-note-actions">
              {note && (
                <button className="stock-note-delete" onClick={() => { onSetNote(''); setShowNote(false); }}>Delete</button>
              )}
              <button className="stock-note-save" onClick={handleNoteSave}>Save</button>
            </div>
          </div>
        </div>,
        document.body
