...
const SOURCE_NOTES = {
  ...
  options:
    "Yahoo Finance options chain — unusual activity detected by volume/OI ratio & premium size",
  short: "FINRA short interest data via Yahoo Finance — updated bi-monthly",
  congress:
    "Quiver Quantitative — STOCK Act filings from House & Senate updated daily",
  govcontracts:
    "Quiver Quantitative — federal government contracts awarded to public companies",
  lobbying:
    "Quiver Quantitative — corporate lobbying disclosures filed with Congress",
  darkpool:
    "Quiver Quantitative — FINRA off-exchange (dark pool) volume & short data",
...
export default function SmartMoney({ active, onSelectStock }) {
  ...
  return (
    <main className="smartmoney-main">
      ...
      {subTab === "insider" && (
        <InsiderTrading
          active={active && subTab === "insider"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "options" && (
        <OptionsFlow
          active={active && subTab === "options"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "short" && (
        <ShortInterest
          active={active && subTab === "short"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "congress" && (
        <CongressTrading
          active={active && subTab === "congress"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "govcontracts" && (
        <GovContracts
          active={active && subTab === "govcontracts"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "lobbying" && (
        <Lobbying
          active={active && subTab === "lobbying"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "darkpool" && (
        <DarkPool
          active={active && subTab === "darkpool"}
          onSelectStock={onSelectStock}
        />
      )}
      {subTab === "wsb" && (
        <WallStreetBets
          active={active && subTab === "wsb"}
          onSelectStock={onSelectStock}
        />
      )}
    </main>
