...
function FilterDropdown({ label, options, value, isActive, onChange, openKey, onToggle, filterKey }) {
  ...
  return (
    <div className="screener-filter">
      ...
      >
        ...
        <svg className="screener-filter-chevron" width="10" height="6" viewBox="0 0 10 6">
      ...
      {isOpen && createPortal(
        <>
          ...
          >
            <div className="screener-dropdown-header">{label || 'Sort by'}</div>
            {options.map(opt => (
              <button
                key={opt.value}
              ...
              >
                ...
                {opt.value === value && (
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M3 8l3.5 3.5L13 5" stroke="var(--green-primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                )}
              </button>
            ))}
...
export default function Screener({ active, onSelectStock, isFavorite, onToggleFavorite }) {
  ...
  const activeFilterCount = [
    filters.marketCap, filters.change, filters.volume, filters.price, filters.sector,
      ...
      <div className="screener-header">
        <div className="screener-header-left">
          ...
          <span className="screener-count">
            {loading ? 'Scanning...' : hasFetched ? `${stocks.length} results` : ''}
            {!loading && hasFetched && totalCount > stocks.length ? ` of ${totalCount.toLocaleString()}` : ''}
          </span>
        </div>
        {activeFilterCount > 0 && (
      ...
      <div className="screener-results">
        ...
        {!loading && hasFetched && stocks.length === 0 && !error && (
          <div className="screener-empty">
            <div className="screener-empty-icon">{activeFilterCount > 0 ? '\uD83D\uDD0D' : '\uD83D\uDCC8'}</div>
            <div className="screener-empty-title">
              {activeFilterCount > 0 ? 'No stocks match your filters' : 'No results'}
            </div>
            <div className="screener-empty-text">
              {activeFilterCount > 0 ? 'Try relaxing some criteria' : 'Try adjusting the sort order'}
            </div>
          </div>
        )}
        {error && (
