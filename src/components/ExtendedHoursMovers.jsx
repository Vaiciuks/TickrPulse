...
function MoverRow({ stock, rank, type, onClick }) {
  ...
  return (
    <button
      className={`movers-row${flash ? ` ${flash}` : ""}`}
    ...
    >
      ...
      <div className="movers-row-data">
        <span className={`movers-row-price${flash ? ` ${flash}` : ""}`}>
          ${formatPrice(stock.price)}
        </span>
        <span className="movers-row-mcap">
          {formatMarketCap(stock.marketCap)}
        </span>
      </div>
      <div className="movers-row-right">
        <span
          className={`movers-row-change ${isGainer ? "positive" : "negative"}`}
        >
          {isGainer ? "+" : ""}
          {stock.change?.toFixed(2)}%
        </span>
        <span className="movers-row-volume">
          ...
          <div className="movers-list">
            {[1, 2, 3, 4, 5, 6].map((n) => (
              <div key={n} className="movers-skeleton-row">
                ...
                <div
                  className="skeleton-line"
                  style={{ width: 42, height: 18, borderRadius: 9 }}
                />
              </div>
            ))}
          </div>
...
export default function ExtendedHoursMovers({ active, onSelectStock }) {
  ...
  const apiSession =
    detectedSession === "pre"
      ...
      : detectedSession === "market"
        ? "pre" // show most recent pre-market data
        : "post"; // post or closed
  ...
  const pollingStopped =
    (session === "pre" && detectedSession !== "pre") ||
    (session === "post" && detectedSession !== "post");
  const { gainers, losers, loading, lastUpdated } = useMovers(
    active,
    session,
    isClosed || isMarketOpen || pollingStopped,
  );
      ...
      <div className="movers-header">
        <div className="movers-header-left">
          ...
          <span className="movers-status">
            <span
              className={`movers-status-dot${isMarketOpen ? " movers-status-dot--market" : isClosed ? " movers-status-dot--closed" : ""}`}
            />
            {statusLabel}
          </span>
        </div>
        ...
        <div className="movers-columns">
          <div className="movers-section">
            <div className="movers-section-header">
              <span className="movers-section-title movers-section-title--gain">
                Gainers
              </span>
            ...
            <div className="movers-list">
              {gainers.length > 0 ? (
                ...
                <div className="movers-empty">
                  No {sessionLabel.toLowerCase()} gainers
                </div>
              )}
            </div>
          </div>
          <div className="movers-section">
            <div className="movers-section-header">
              <span className="movers-section-title movers-section-title--lose">
                Losers
              </span>
            ...
            <div className="movers-list">
              {losers.length > 0 ? (
                ...
                <div className="movers-empty">
                  No {sessionLabel.toLowerCase()} losers
                </div>
              )}
            </div>
          </div>
