...
function isSpecialSymbol(symbol) {
  ...
  return symbol.includes('=') || symbol.startsWith('^') || symbol.includes('-USD');
}
...
export default function StockLogo({ symbol, size = 28 }) {
  ...
  if (isSpecial || override) {
    const abbr = override?.abbr || (symbol || '?').replace(/[=^]/g, '').slice(0, 3);
    const color = override?.color || getColor(symbol || '');
    const fontSize = size * (abbr.length > 3 ? 0.30 : abbr.length > 2 ? 0.34 : 0.42);
    return (
      <span
